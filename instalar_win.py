#!/usr/bin/env python3
"""
VIGIA — Instalador gráfico (Windows)
...
Equivalente exacto de instalar.py para Windows.
Reemplaza: scripts .sh → scripts .bat
"""
...
class InstaladorVIGIA:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title('VIGIA — Instalador (Windows)')
    ...
    def _build_ui(self):
        ...
        tk.Label(cab, text='Sistema de supervisión de aula (Windows)',
    ...
    def _instalar(self):
        ...
        if tipo == 'servidor':
            script = os.path.join(SCRIPT_DIR, 'instalar_servidor_win.bat')
            cmd = ['cmd', '/c', script]
            self._log('▶ Instalando VIGIA Servidor (Windows)...\n\n')
        else:
            script = os.path.join(SCRIPT_DIR, 'instalar_cliente_win.bat')
            cmd = ['cmd', '/c', script, ip] if ip else ['cmd', '/c', script]
            self._log(f'▶ Instalando VIGIA Cliente (Windows)'
